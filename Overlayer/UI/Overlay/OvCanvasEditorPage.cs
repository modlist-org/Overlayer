...
public class OvCanvasEditorPage : IDisposable {
    ...
    public OvCanvasEditorPage(Transform parent, Action onBack) {
        onBackAction = onBack;

        GameObject = new(nameof(OvCanvasEditorPage));
    ...
    private void RebuildInspector() {
        ...
        if(selectedObject == null) {
            var infoRow = GenerateUI.Row(inspectorContent, 60f);
            var infoTxt = GenerateUI.AddText(infoRow, true);
            infoTxt.text = "Select an object to inspect";
            infoTxt.fontSize = 20f;
            infoTxt.alignment = TextAlignmentOptions.Center;
            infoTxt.color = new Color(1, 1, 1, 0.4f);
