...
public class ColorPicker : UIBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler, IMaterialModifier {
    ...
    public void OnPointerUp(PointerEventData eventData) => _pointerDownLocation = PointerDownLocation.Outside;
    ...
    public float InverseLerpUnclamped(float min, float max, float value) => (value - min) / (max - min);
}